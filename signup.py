...
def signup():
    ...
    def save_to_excel():
        ...
        if not all([employee_name, employee_username, employee_password, employee_confirm_password, employee_designation, employee_gender, employee_phone, employee_address]):
            messagebox.showerror("Error", "Please Fill The Fields")
            return
        elif employee_password == employee_confirm_password:
            data = {'Name': [employee_name], 'Username': [employee_username], 'Password': [employee_password], 'Designation': [employee_designation], "Gender": [employee_gender], "Phone": [employee_phone], "Address": [employee_address]}
